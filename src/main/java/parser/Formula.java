...
public class Formula {
    ...
    public Formula(String expression) throws SDNFException {
            if ("1".equals(expression) || "0".equals(expression)) {
                result = false;
                throw new SDNFException("1".equals(expression) ? 14 : 13);
            }
            parser = new Parser(expression);
            formula = expression;
            tree = parser.getTree();
            ELEMENTS = new ArrayList<>(parser.getELEMENTS());
            createTruthTable();//составление таблицы истинности по выражению
...
//            output();
    ...
    private void createTruthTable() throws SDNFException {
...
//        System.out.println("Don?e!");
    ...
    private boolean determineValue(int[] value, ExpressionTree tree) throws SDNFException {
        switch (tree.getOperation()) {
            ...
            case "": {
                ...
                int v = value[list.indexOf(tree.getExpression())];
                return v == 1;
            }
            default: {
                throw new SDNFException(13);
            }
        }
    }
...
//        }
    ...
    private String createAtom(int countElements, int[] rowTruthTable) {
        ...
        for (int i = 0; i < countElements; i++) {
            atom.append((rowTruthTable[i] == 1) ? ELEMENTS.get(i) : ("(!" + ELEMENTS.get(i) + ")"));
            if (count != 0) {
                atom.append(")");
            }
            atom.append(SIGN);
